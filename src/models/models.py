...
class BiEncoderAttentionWithRationaleClassification(nn.Module):
	...
	# AQUI PASAMOS POR LOS ENCODERS
		...
		# AHORA LA SECUENCIA RESULTANTE DE LOS ENCODERS IRÁ HACIA ATTENTION, 
		# PERO PRIMERO PASARÁ POR UNA CAPA DROPOUT
		...
		##RobertaClassifier te entrega el level 
		...
		if empathy_labels is not None:
			loss_fct = CrossEntropyLoss()
			empathy_labels = torch.tensor(empathy_labels, dtype=torch.long)
